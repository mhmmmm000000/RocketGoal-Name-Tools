...
interface GenerateBody {
  ...
  // Optional: access token from OAuth, used for re-verification
  accessToken?: string;
}
...
// Shame payload — used when user un-follows / un-stars after unlocking
...
function buildCurvedColoredName(name: string, colors: string[]): string {
  ...
  return chars
    .map((char, i) => {
      ...
      const voffset = Math.round(Math.abs(t) * maxVoffset);
...
function buildFlatColoredName(name: string, colors: string[]): string {
  return name
    .split("")
    .map((char, i) => `<color=${colors[i % colors.length]}>${char}</color>`)
    .join("");
}

function buildTargetName(body: GenerateBody, isShame: boolean): string {
  if (isShame) {
    return SHAME_PAYLOAD;
  }
  ...
  let namePart: string;
  if (body.curve) {
    namePart = `<size=100><b>${buildCurvedColoredName(cleanName, colors)}</b></size>`;
  } else {
    namePart = `<size=100><b>${buildFlatColoredName(cleanName, colors)}</b></size>`;
  }

  const parts: string[] = [];

  if (body.includeSubTo) {
    parts.push(`<size=40><color=#FF1744><b>SUB TO</b></color>`);
  }
  ...
  if (body.includeUrl && body.youtubeHandle) {
    ...
    parts.push(`<size=25><color=#FFFFFF><b>youtube.com/@${handle}</b></color>`);
  }
...
// Simple obfuscation — XOR + base64. Not crypto-secure, but raises the bar
// so casual users can't read the technique from someone else's console.
...
function buildWrappedScript(targetName: string, obfuscated: boolean): string {
  const innerScript = `const TARGET_NAME = ${JSON.stringify(targetName)};
...
console.log('%c\\u2192 Now change your name in the game UI to trigger it', 'color:#fff');`;
  ...
  // Wrap with de-obfuscation layer
  const payload = obfuscate(innerScript);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Re-verify access if token provided
    let isShame = false;
    let verifyReason = "";
    if (body.accessToken) {
      ...
      // No token = unverified user. Return preview-only mode.
    ...
    const script = buildWrappedScript(targetName, true);
...
// Separate endpoint to just generate a preview (no script, no verification)
export async function PUT(request: NextRequest) {
  try {
    ...
    const nameHtml = body.curve
      ? buildCurvedColoredName(cleanName, colors)
